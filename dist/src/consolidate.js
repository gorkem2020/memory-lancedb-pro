...
export function computeConsolidateCostPreview(units, mergeChunkSize = CONSOLIDATE_MERGE_BATCH_MAX_SIZE) {
 ...
 * LLM calls.
 */
async function buildMergePlanContentsBatch(deps, jobs, mergeChunkSize = CONSOLIDATE_MERGE_BATCH_MAX_SIZE, log) {
...
export async function runConsolidate(deps, options) {
    ...
    const mergeChunkSize = Math.min(50, Math.max(1, Math.floor(options.mergeChunkSize ?? CONSOLIDATE_MERGE_BATCH_MAX_SIZE)));
    ...
    for (const unit of units) {
        fingerprintByUnit.set(unit, computeClusterFingerprint(unit.members.map((m) => ({ id: m.entry.id, metadata: m.entry.metadata }))));
    }
    ...
    if (units.length > 0) {
        ...
        if (decideCallFailed) {
            deps.log?.(`memory-consolidate: consolidate-decide call returned no response (provider error or timeout); ${units.length} cluster(s) left undecided`);
        }
