...
export function resolveAdmissionModel(params) {
    const normalize = (modelRef) => params.transport === "host" ? modelRef : normalizeAdmissionModelRef(modelRef);
...
export class AdmissionController {
    ...
    async evaluateChunk(chunk) {
        ...
        if (scoreableIndices.length > 0) {
            const utilities = await this.scoreUtilityBatch(scoreableIndices.map((i) => chunk[i].candidate), chunk[0].conversationText);
