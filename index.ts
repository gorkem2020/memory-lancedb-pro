          ...
          if (smartExtractor) {
            ...
            if (cumulativeCount >= minMessages) {
              ...
              if ((stats.boundarySkipped ?? 0) === 0) {
                ...
                // same way (issue #417 Fix #5/#9), or the next turn re-fires
                // on just one new message (ingress-fed) or re-reads the
                // whole history (history-carrying) instead of waiting for a
                // fresh window.
                autoCaptureSeenTextCount.set(
                  sessionKey,
                  pendingIngressTexts.length > 0 ? 0 : eligibleTexts.length,
