...
function subsetRatio(inner: Set<string>, outer: Set<string>): number {
  if (inner.size === 0) return 0;
  let contained = 0;
  for (const token of inner) {
    if (outer.has(token)) contained++;
  }
...
export function isNearIdenticalEcho(candidateText: string, manualText: string): boolean {
  ...
  if (manualTokens.size < MIN_CONTAINMENT_TOKENS) return false;

  if (candidate.includes(manual) || manual.includes(candidate)) return true;
  ...
  if (
    subsetRatio(manualTokens, candidateTokens) >= MANUAL_ECHO_SUBSET_THRESHOLD ||
...
export class ManualEchoLedger {
  ...
  record(agentId: string | undefined, text: string): void {
  ...
  match(agentId: string | undefined, candidateText: string): string | null {
    const key = agentId?.trim() || DEFAULT_AGENT_BUCKET;
    const ring = this.byAgent.get(key);
    if (!ring || ring.length === 0) return null;
    for (let i = ring.length - 1; i >= 0; i--) {
      if (isNearIdenticalEcho(candidateText, ring[i])) return ring[i];
    }
    return null;
  }
