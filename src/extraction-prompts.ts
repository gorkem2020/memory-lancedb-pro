...
- Always set the top-level "conversation_register" field, and tag every memory's "grounding" field, per the Conversational Grounding rules above`;
  ...
  const readingOrder = `Read the conversation in chronological order, top to bottom, and understand it as a whole before extracting anything. ${
    assistantEligible
      ...
      : "Extraction still draws only from the <user_message> blocks — but interpret each one through your understanding of the full conversation, not in isolation."
  } As you read, notice which stretches describe the real world and which step into a supposed, imagined, or made-up frame.`;
