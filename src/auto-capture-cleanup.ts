 ...
 */
export function countProtectedReferentPrefix(
  turns: ConversationTurn[],
  referentTurns: ReadonlySet<ConversationTurn>,
): number {
  let count = 0;
  for (const turn of turns) {
    if (turn.contextOnly === true) continue;
    if (!referentTurns.has(turn)) break;
    count++;
  }
  return count;
}
