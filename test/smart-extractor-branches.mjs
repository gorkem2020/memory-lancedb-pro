...
async function runAssistantContextModeScenario() {
  ...
  try {
    ...
    await runAgentEndHook(
      ...
      {
        success: true,
        // History-carrying agent_end delivers the WHOLE session each turn
        // (see issue #417 Fix #9): turn 2 must resend turn 1's messages too,
        // not just this turn's delta, or the watermark rollback on the
        // turn-1 skip (which resets the cursor to 0 expecting the next
        // turn's full-history slice) makes turn 2 look like a fresh
        // 1-message turn instead of the accumulated 2.
        messages: [
          { role: "user", content: "我的名字是小明" },
          { role: "assistant", content: "很高兴认识你，小明！你喜欢什么运动？" },
