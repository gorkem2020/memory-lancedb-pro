...
async function runAssistantContextModeScenario() {
  ...
  try {
    ...
    // NOTE (2026-07-16): the auto-capture counter rolls back to its pre-turn
    // value on a skip when there is no pending-ingress text (index.ts's
    // `pendingIngressTexts.length === 0` branch), so a turn 2 agent_end call
    // must carry the FULL accumulated session history, not just this turn's
    // delta -- matching the established pattern in
    // test/autocapture-watermark-reset.test.mjs ("Turn 2: agent_end again
    // carries the FULL history"). Sending only the delta would make turn 2
    // look like a fresh 1-message turn (cumulative stays at 1 instead of the
    // correct accumulated 2).
    await runAgentEndHook(
      api,
      {
        success: true,
