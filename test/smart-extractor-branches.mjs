...
async function runAssistantContextModeScenario() {
  ...
  try {
    ...
    // Turn 2 carries the FULL accumulated history (turn 1 + turn 2 messages),
    // matching how a no-message_received, agent_end-driven flow actually
    // redelivers state each turn (see fix/autocapture-watermark-reset's own
    // "history flow" test). A delta-only turn 2 collides with the
    // below-threshold rollback logic (autocapture-fallback-gating), which
    // treats a turn with no separate pendingIngressTexts entry as
    // history-carrying and rolls the watermark back after a deferred turn.
    await runAgentEndHook(
      api,
      {
        success: true,
