...
describe("SmartExtractor batch register signal (grounding v2)", () => {
  ...
  it("missing register (legacy payload) WITH a constructed sibling still arms the batch-wide durable wipe", async () => {
    ...
    // v3: MISLABELED_FICTION_CANDIDATES[2] is now correctly tagged "real" (about-the-fiction),
    // so it no longer serves as a constructed sibling — CONSTRUCTED_PLOT_EVENT (within-the-
    // fiction) arms the wipe instead, and GAME_CANDIDATES[2] (a real, about-the-fiction session
    // note) is the survivor that proves the wipe only demotes durables, not events.
    const store = makeStore();
    const llm = makeLlm([...FACTUAL_CANDIDATES, CONSTRUCTED_PLOT_EVENT, GAME_CANDIDATES[2]]); // no conversation_register field
    const extractor = makeExtractor(makeEmbedder(), llm, store);
