  ...
  it("buildUtilityPrompt interpolates grounding and names all six registers (structural check)", async () => {
    const llm = {
      ...
      async completeJson(prompt, mode, systemPrompt) {
        if (mode === "admission-utility") {
          this.prompts.push({ user: prompt, system: systemPrompt });
    ...
    assert.match(user, /Grounding: constructed/);
    assert.match(user, /Conversation register: fiction/);
    for (const register of ["profile", "preferences", "entities", "events", "cases", "patterns"]) {
      assert.match(system, new RegExp(register), `utility prompt must name the ${register} register`);
    }
    assert.match(system, /roleplay/i, "utility prompt must carry the fiction guidance");
  });
});
