...
describe("AdmissionController grounding awareness (grounding v2)", () => {
  ...
  it("buildUtilityPrompt carries the grounding rule paragraph and names all six registers (structural check)", async () => {
    ...
    // deterministic constructed-durable short-circuit (this candidate's
    // "events" category is exempt from it, since a session-scoped events
    // note about the fiction happening may still score moderately) plus this
    // prose rule paragraph, both carried in the system prompt.
