...
describe("SmartExtractor batch admission integration", () => {
  it("scores admission utility for a whole extraction batch with exactly one LLM call", async () => {
    ...
    const admissionControlConfig = normalizeAdmissionControlConfig({ enabled: true, utilityMode: "batch" });
    ...
    const extractor = new SmartExtractor(makeStore(), makeEmbedder(), llm, {
      ...
      admissionController: createAdmissionController(makeStore(), llm, admissionControlConfig),
      log() {},
      debugLog() {},
    });
