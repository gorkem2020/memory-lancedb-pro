...
describe("resolveAdmissionModel", () => {
  ...
  // Structural fix for the host-managed runtime LLM transport
  // (https://github.com/CortexReach/memory-lancedb-pro/issues/901): the
  // openrouter/ prefix-strip above is a direct-path-only accommodation for
  // the bundled OpenAI-compatible client. When transport: "host" is
  // configured, the full core-style catalog reference must reach
  // runtime.llm.complete unmodified so the host can resolve provider
  // routing itself.
  it("passes the reflection model through unmodified when transport is host", () => {
    const admissionControl = normalizeAdmissionControlConfig({ enabled: true, modelAffinity: "lane" });
    ...
    assert.equal(reflection, "openrouter/anthropic/claude-opus-4-8");
  });

  it("passes an explicit admissionControl.model override through unmodified when transport is host", () => {
    const admissionControl = normalizeAdmissionControlConfig({
      enabled: true,
      modelAffinity: "lane",
    ...
    assert.equal(other, "openrouter/anthropic/claude-opus-4-8");
  });

  it("still normalizes for the direct transport when transport is omitted (default unchanged)", () => {
    const admissionControl = normalizeAdmissionControlConfig({ enabled: true, modelAffinity: "lane" });
