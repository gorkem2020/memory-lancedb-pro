...
describe("AdmissionController.evaluateBatch", () => {
  ...
  it("fences the batch few-shot example so it cannot be misread as an instruction for the live batch", async () => {
    ...
    assert.ok(capturedSystemPrompt, "expected a batch-utility call");
    assert.doesNotMatch(
      capturedSystemPrompt,
      /Expected response:/,
      "the few-shot label must not read as an instruction the model is expected to follow literally"
    );
    assert.match(capturedSystemPrompt, /Example response:/);
    assert.match(capturedSystemPrompt, /--- EXAMPLE \(not your current batch\) ---/);
    assert.match(capturedSystemPrompt, /--- END EXAMPLE ---/);
  });
});
